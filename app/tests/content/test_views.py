...
def test_get_retrieve_poll(api_client, add_answer):
    ...
    response = api_client.get(
        f"/polls/{answer.question.poll.id}/"
    )
...
def test_update_poll(api_client, add_admin_token, add_poll):
    ...
    response = api_client.patch(
        f"/polls/{poll.id}/", {"title": "Новый опрос"})
...
def test_update_answer(api_client, add_admin_token, add_answer):
    ...
    response = api_client.patch(
        f"/answers/{answer.id}/", {"variant": "Новый вариант"}
    )
...
@pytest.mark.django_db
def test_fail_add_select_answer_invalid_auth_user(api_client, add_user_token, add_answer):
    token = add_user_token()
    answer = add_answer("Вариант 1")
    api_client.force_authenticate(token=token, user=token.user)
...
@pytest.mark.django_db
def test_get_all_passed_polls_by_auth_user(api_client, add_answer, add_user_token, add_auth_user_answer):
    token = add_user_token()
    user_answer_1 = add_auth_user_answer(add_answer=add_answer("Первый вариант"), token=token)
    user_answer_2 = add_auth_user_answer(add_answer=add_answer("Второй вариант"), token=token)

    response = api_client.get("/passed-polls/")

    assert response.status_code == 200
    assert response.data[0]["questions"][0]["answers"][0]["variant"] == user_answer_1.answer.variant
    assert response.data[1]["questions"][0]["answers"][0]["variant"] == user_answer_2.answer.variant


@pytest.mark.django_db
def test_get_all_passed_polls_by_anonymous_user(api_client, add_answer, add_anonymous_user_answer):
    anonymous_id = 12345
    user_answer1 = add_anonymous_user_answer(anonymous_id=anonymous_id, add_answer=add_answer("Вариант 1"))
    user_answer2 = add_anonymous_user_answer(anonymous_id=anonymous_id, add_answer=add_answer("Вариант 2"))

    response = api_client.get(f"/passed-polls/anonymous/{anonymous_id}/")

    assert response.status_code == 200
    assert response.data[0]["questions"][0]["answers"][0]["variant"] == user_answer1.answer.variant
    assert response.data[1]["questions"][0]["answers"][0]["variant"] == user_answer2.answer.variant
